    ...
    return (
        ...
        {menuOpen && (
          ...
          {user ?  
            ...
            user && (
                <div key={item.key}>
              <button className={active === item.key ? styles.openLabel : styles.userInfoBtn} type="button" onClick={() => handleToggle(item.key)}>{active === item.key ? item.openLabel : item.label} {active === item.key ? <i className="fa-solid fa-xmark"></i> : null}</button>
